...
async function readJSON<T>(res: Response): Promise<T> {
  ...
  throw new Error(`La ruta devolvió contenido no-JSON (${res.status}). ${text.slice(0, 180)}`);
}
...
export default function ContactForm() {
  ...
  async function requestCode() {
    ...
    try {
      ...
      if (!res.ok || !data.ok || !data.token) throw new Error(data.error || "No se pudo enviar el código.");
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setName(""); setEmail(""); setSubject(""); setMessage(""); setCode("");
    } catch (err) {
      setState({ status: "error", message: getErr(err) });
    }
  }
    ...
    <div className="relative">
      ...
      <div className="relative surface max-w-3xl mx-auto p-6 md:p-8">
        ...
        <form onSubmit={onSubmit} className="grid grid-cols-1 gap-4">
          ...
          {/* Mensajes de estado (sin reveal para no depender del viewport) */}
          {state.status === "error" && (
            <div className="text-red-400 text-sm">{state.message}</div>
          )}
          {state.status === "success" && (
            <div className="text-green-400 text-sm">
              ¡Gracias! Tu email ha sido verificado y el mensaje enviado.
            </div>
          )}
